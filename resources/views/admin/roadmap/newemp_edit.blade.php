        ...
        <div class="content-wrapper">
            <div class="content-header">
                <div class="container-fluid">
                    <div class="d-flex align-items-center">
                        <div class="">
                            <h4 class="m-0">แก้ไขRoadmapสำหรับพนังงานใหม่</h4>
                        </div>
                        <div class="ml-3">
                            <a href="{{route('admin')}}">
            ...
            <div class="container mt-4">
                <div class="card">
                    ...
                    @endif
                    <div class="card-body">
                        <form action="{{ route('roadmap.newemp.update',$newEmpCourseEdit->id) }}" method="POST" enctype="multipart/form-data" id="roadmapForm">
                            @csrf
                            <div class="row">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-4">
                                    <div class="sticky-top" style="top: 20px;">
                                        <div class="card shadow-sm">
                                            ...
                                            @php
                                                // 1. ดึงวิชาที่มีอยู่ใน Roadmap นี้ออกมา แล้วจัดกลุ่มตาม milestone_days (30, 60, 90...)
                                                // เราจะใช้ $groupedData นี้เป็น "แหล่งข้อมูลหลัก" ในการหยิบวิชามาโชว์ในแต่ละเดือน
                                                $groupedData = $newEmpCourseEdit->roadmapCourse->groupBy('milestone_days');

                                                // 2. ดึงโครงสร้างเดือน (1-4) มาจาก Model ที่เราทำไว้
                                                $courseInventory = $groupedData->get("") ?? collect();
                                            @endphp
                                            <div class="card-body p-2 drag-area" id="courseInventory" style="max-height: 70vh; overflow-y: auto;">
                                                @foreach ( $courseInventory as $courseEdit )
                                                    <div class="list-group-item list-group-item-action mb-1 p-2 rounded draggable-item" data-id="{{ $courseEdit->course_id }}">
                                                        <small class="text-muted">หลักสูตร:</small> | {{ $courseEdit->course->course_title }}
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                        ...
                                        <div class="card mb-2">
                                            ...
                                            <div id="collapse-{{$day}}" class="collapse show">
                                                <div class="card-body p-3 drag-area" data-milestone="{{$day}}" id="month-{{$day}}" style="min-height: 80px; background-color: #fcfcfc; border: 1px dashed #ccc;">
                                                    @php
                                                        // ไปหยิบวิชาของเดือนนี้ (ตามเลข $day) ออกมาจากกลุ่มข้อมูล
                                                        $coursesInMonth = $groupedData->get($day) ?? collect();
                                                    @endphp
                                                    @foreach($coursesInMonth->sortBy('order') as $item)
                                                        <div class="list-group-item list-group-item-action mb-1 p-2 rounded draggable-item" data-id="{{ $item->course_id }}">
                                                            <small class="text-muted">หลักสูตร:</small> | {{ $item->course->course_title }}
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
