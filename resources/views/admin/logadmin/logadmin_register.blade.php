	...
	<div class="container-fluid fluid menu-left">
		...
		<div id="wrapper">
			...
			<!-- <div class="span-19"> -->
				<div id="content">
					<ul class="breadcrumb">
						<li><a href="/admin/index.php">หน้าหลัก</a></li> » <li>Log ทั่วไปการตรวจสอบสมัครสมาชิก</li>
					</ul><!-- breadcrumbs -->
					...
					<div class="innerLR">
						<div class="widget" style="margin-top: -1px;">
							<div class="widget-head">
								<h4 class="heading glyphicons show_thumbnails_with_lines"><i></i> Log ทั่วไปการตรวจสอบสมัครสมาชิก</h4>
							</div>
									...
									<div style="margin-top: -1px;" id="News-grid" class="grid-view">
										...
										<div class="pagination pull-right">
											<ul class="pagination margin-top-none" id="yw0">
												<li class="first"><a href="{{url('logregister')}}">&lt;&lt; หน้าแรก</a></li>
												@if ($logregister->currentPage() > 1)
												<li class="previous"><a href="{{ $logregister->previousPageUrl() }}" class="pagination-link">หน้าที่แล้ว</a></li>
												@endif
												@for ($i = max(1, $logregister->currentPage() - 3); $i <= min($logregister->lastPage(), $logregister->currentPage() + 3); $i++)
												<li class="page"><a href="{{ $logregister->url($i) }}" class="pagination-link {{ ($i == $logregister->currentPage()) ? 'active' : '' }}">{{ $i }}</a></li>
												@endfor
												@if ($logregister->currentPage() < $logregister->lastPage())
												<li class="next"><a href="{{ $logregister->nextPageUrl() }}" class="pagination-link">หน้าถัดไป</a></li>
												@endif
												@if ($logregister->currentPage() == $logregister->lastPage())
												<li class="last"><a href="{{ $logregister->lastPage() }}"  class="pagination-link">หน้าสุดท้าย &gt;&gt;</a></li>
												@endif
											</ul>
										</div>
